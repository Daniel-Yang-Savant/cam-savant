...
export async function GET(request: NextRequest) {
  ...
  const destination = isEnglish ? '/en/perioperative-rehab' : '/perioperative-rehab'
  const lockedDestination = isEnglish
  ...
  if (!validToken || (!hasValidToken && !hasValidGrant)) {
    return NextResponse.redirect(new URL(lockedDestination, request.url))
  }
