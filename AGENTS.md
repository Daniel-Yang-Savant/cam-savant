...
- `/admin-login` — 管理員登入頁（設定 HttpOnly cookie `admin_token`，有效期 6 小時）
...
執行任何 SEO、醫療文章新增或更新、網站文案、AI 搜尋、看診 CTA、分析事件或成效檢討前，必須先閱讀並遵守：

- [`CAM-SAVANT-SEO-CONTENT-WORKFLOW.md`](./CAM-SAVANT-SEO-CONTENT-WORKFLOW.md)

固定優先順序為：醫療正確性與患者安全 → 來源可驗證性 → 搜尋意圖 → 使用體驗 → 合理轉換 → 流量成長。

預設採最小有效修改，不為 SEO 大幅改寫已審核正文；所有新增醫療宣稱與引用都必須可驗證，分析事件不得傳送個人資料、諮詢內容或健康資訊。
